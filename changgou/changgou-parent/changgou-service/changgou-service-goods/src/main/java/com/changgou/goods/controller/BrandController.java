...
public class BrandController {
     ...
     * 根据分类id查询品牌集合
     */
    @ApiOperation(value = "分类id查询品牌集合",notes = "分类id查询品牌集合",tags = {"BrandController"})
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "path", name = "id", value = "分类id", required = true, dataType = "Integer"),
    })
    @GetMapping(value = "/category/{id}")
    public Result<List<Brand>>  findByCategory(@PathVariable(name = "id") Integer id){
        List<Brand> brands = brandService.findByCategory(id);
        return  new Result<List<Brand>>(true,StatusCode.OK,"操作成功！",brands);
    }
